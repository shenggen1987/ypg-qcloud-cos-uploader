...
const generateKey = config => filePath => {
    ...
    return `${config.prefix}${config.prefix ? '/' : ''}${config.randomPrefix ? uuidv1() : ''}${config.randomPrefix ? '/' : ''}${normalizePath}`
}

const createTaskQueue = config => {
    const excludeReg = new RegExp(config.exclude)
    const taskQueue = utils
        ...
        .filter(filePath => {
            if (config.exclude) {
                return !excludeReg.test(filePath)
            }
            return true
        })
        // 创建上传任务队列
